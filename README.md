...
| LLM / Reasoning | Google Gemini API (`gemini-3.6-flash`) |
...
| Fallback Behavior | `Currently no graceful fallback — if Rime fails, the whole request returns a generic 500 error instead of surfacing the text response` |
...
```bash
git clone https://github.com/niharikahub21/voicelayer.git
cd voicelayer
```
